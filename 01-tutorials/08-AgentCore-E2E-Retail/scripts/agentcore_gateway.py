    ...
    try:
            ...
            "mcp": {
                "lambda": {
                    "lambdaArn": get_ssm_parameter(
                        "/app/retail/agentcore/lambda_arn"
                    ),
        ...
        auth_config = {
            "customJWTAuthorizer": {
                "allowedClients": [
                    get_ssm_parameter(
                        "/app/retail/agentcore/machine_client_id"
                    )
                ],
                "discoveryUrl": get_ssm_parameter(
                    "/app/retail/agentcore/cognito_discovery_url"
                ),
            }
        }

        execution_role_arn = get_ssm_parameter(
            "/app/retail/agentcore/gateway_iam_role"
        )
        ...
        create_response = gateway_client.create_gateway(
            ...
            description="Retail AgentCore Gateway",
        )
        ...
        put_ssm_parameter("/app/retail/agentcore/gateway_id", gateway_id)
        put_ssm_parameter("/app/retail/agentcore/gateway_name", gateway_name)
        put_ssm_parameter(
            "/app/retail/agentcore/gateway_arn", create_response["gatewayArn"]
        )
        put_ssm_parameter(
            "/app/retail/agentcore/gateway_url", create_response["gatewayUrl"]
        )
        put_ssm_parameter(
            "/app/retail/agentcore/cognito_secret",
            get_cognito_client_secret(),
...
def get_gateway_id_from_config() -> str:
    ...
    try:
        return get_ssm_parameter("/app/retail/agentcore/gateway_id")
    except Exception as e:
...
def cli(ctx):
    ...
    Create and delete AgentCore gateways for the retail application.
    """
...
def delete(gateway_id, confirm):
    ...
    if delete_gateway(gateway_id):
        ...
        delete_ssm_parameter("/app/retail/agentcore/gateway_id")
        delete_ssm_parameter("/app/retail/agentcore/gateway_name")
        delete_ssm_parameter("/app/retail/agentcore/gateway_arn")
        delete_ssm_parameter("/app/retail/agentcore/gateway_url")
        delete_ssm_parameter("/app/retail/agentcore/cognito_secret")
