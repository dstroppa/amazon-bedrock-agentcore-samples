...
memory_name = "RetailMemory"


def create_or_get_memory_resource():
    try:
        memory_id = get_ssm_parameter("/app/retail/agentcore/memory_id")
        ...
        try:
            strategies = [
                {
                    StrategyType.USER_PREFERENCE.value: {
                        ...
                        "namespaces": ["retail/customer/{actorId}/preferences"],
                    }
                },
                {
                    StrategyType.SEMANTIC.value: {
                        ...
                        "namespaces": ["retail/customer/{actorId}/semantic"],
                    }
                },
            ]
            ...
            response = memory_client.create_memory_and_wait(
                name=memory_name,
                description="Shopping assistant agent memory",
                strategies=strategies,
                event_expiry_days=90,  # Memories expire after 90 days
            )
            memory_id = response["id"]
            try:
                put_ssm_parameter("/app/retail/agentcore/memory_id", memory_id)
            except:
                raise
            return memory_id
        except:
...
def delete_memory(memory_hook):
    try:
        ...
        ssm_client.delete_parameter(Name="/app/retail/agentcore/memory_id")
    except Exception:
        pass


class ShoppingAssistantMemoryHooks(HookProvider):
    """Memory hooks for shopping assistant agent"""
    ...
    def retrieve_customer_context(self, event: MessageAddedEvent):
        """Retrieve customer context before processing shopping query"""
    ...
    def save_shopping_interaction(self, event: AfterInvocationEvent):
        """Save shopping interaction after agent response"""
        try:
            messages = event.agent.messages
            if len(messages) >= 2 and messages[-1]["role"] == "assistant":
                ...
                if customer_query and agent_response:
                    # *** AGENTCORE MEMORY USAGE *** - Save the shopping interaction
                    ...
                    logger.info("Saved shopping interaction to memory")

        except Exception as e:
            logger.error(f"Failed to save shopping interaction: {e}")

    def register_hooks(self, registry: HookRegistry) -> None:
        """Register shopping assistant memory hooks"""
        registry.add_callback(MessageAddedEvent, self.retrieve_customer_context)
        registry.add_callback(AfterInvocationEvent, self.save_shopping_interaction)
        logger.info("Shopping assistant memory hooks registered")
