...
username = "testuser"
secret_name = "retail_assistant"

role_name = f"RetailAssistantBedrockAgentCoreRole-{REGION}"
policy_name = f"RetailAssistantBedrockAgentCorePolicy-{REGION}"
...
def save_shopping_assistant_secret(secret_value):
    ...
    try:
        secrets_client.create_secret(
            ...
            Description="Secret containing the Cognito Configuration for the Retail Assistant",
        )
...
def get_shopping_assistant_secret():
...
def delete_shopping_assistant_secret():
...
def setup_cognito_user_pool():
    ...
    try:
        ...
        save_shopping_assistant_secret(json.dumps(cognito_config))
...
def create_agentcore_runtime_execution_role():
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Resource": [
                    ...
                    f"arn:aws:bedrock-agentcore:{region}:{account_id}:workload-identity-directory/default/workload-identity/shopping_assistant_agent-*",
                ],
            },
            {
            ...
            },
            {
                ...
                "Resource": [
                    f"arn:aws:ssm:{region}:{account_id}:parameter/app/retail/agentcore/memory_id"
                ],
            },
        ],
    }
    ...
    try:
        ...
        role_response = iam.create_role(
            ...
            Description="IAM role for Amazon Bedrock AgentCore Retail Assistant with required permissions",
        )
        ...
        put_ssm_parameter(
            "/app/retail/agentcore/runtime_execution_role_arn",
...
def runtime_resource_cleanup():
    try:
        ...
        for repo in repositories['repositories']:
            if 'bedrock-agentcore-retail_assistant' in repo['repositoryName']:
...
def delete_observability_resources():
    # Configuration
    log_group_name = "agents/retail-assistant-logs"
